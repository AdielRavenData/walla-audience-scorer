#!/usr/bin/env python3
"""
Verticals Audience Summary Generator

This script creates a summary table of vertical counts per user from the users_with_audience_score table
and appends the results to a verticals_audience_summary table in BigQuery.

Usage:
    python verticals_table.py
"""
...
class VerticalsAudienceSummaryGenerator:
    """
    Generator for creating verticals audience summary from BigQuery data.
    """
    ...
    def __init__(self, project_id: str = "wallabi-169712"):
        ...
        logger.info(f"Initialized VerticalsAudienceSummaryGenerator for project: {project_id}")
    
    def generate_verticals_summary(self, run_id: int, audience: str) -> pd.DataFrame:
        """
        Generate verticals audience summary from BigQuery data.
        ...
        Returns:
            pd.DataFrame: DataFrame with verticals summary data
        """
        try:
            logger.info(f"Generating verticals audience summary with run_id: {run_id}, audience: {audience}")
            
            # SQL query to get vertical counts per user
            query = f"""
            WITH run_users AS (
            ...
            GROUP BY 1, 2, 3, 4
            ORDER BY a.user_unique_id, vertical_count DESC
            """
            ...
            logger.info(f"Query completed! Fetched {len(df):,} vertical summary records")
            print(f"✅ BigQuery query completed! Fetched {len(df):,} vertical summary records")
            
            return df
            
        except Exception as e:
            logger.error(f"Error generating verticals summary: {e}")
            raise
    
    def append_to_bigquery_table(self, df: pd.DataFrame, table_name: str = "verticals_audience_summary") -> None:
        """
        Append results to BigQuery table.
        
        Args:
            df: DataFrame with verticals summary data
    ...
    def run(self, run_id: int, audience: str, table_name: str = "verticals_audience_summary") -> pd.DataFrame:
        """
        Main method to generate and upload verticals audience summary.
        ...
        """
        try:
            print("🚀 Starting Verticals Audience Summary generation...")
            
            # Step 1: Generate summary data
            print("📊 Step 1/2: Generating verticals summary from BigQuery...")
            summary_df = self.generate_verticals_summary(run_id, audience)
            ...
            print(f"Unique verticals: {summary_df['vertical_name'].nunique():,}")
            print(f"Average verticals per user: {len(summary_df) / summary_df['user_unique_id'].nunique():.1f}")
            
            logger.info("Verticals audience summary generation complete!")
            return summary_df
            
        except Exception as e:
            logger.error(f"Error in verticals summary generation: {e}")
            raise

def main():
    """Main function to run the verticals audience summary generator."""
    try:
        # Create generator
        generator = VerticalsAudienceSummaryGenerator()
        ...
        table_name = "verticals_audience_summary"
        ...
        print(f"\nStarting verticals summary generation...")
        print(f"Run ID: {run_id}")
        print(f"Audience: {audience}")
        ...
        if not results.empty:
            print(f"\n🎉 Success! Generated {len(results):,} vertical summary records")
        else:
